...
router.post(PATH_LIST, function postParty(request, response, next) {
  ...
  Party.post(body, function onPostParty(error, created) {
    //forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchParty(request, response, next) {
  ...
  Party.patch(_id, patches, function onPatchParty(error, patched) {
    //forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putParty(request, response, next) {
  ...
  Party.put(_id, updates, function onPutParty(error, updated) {
    //forward error
    if (error) {
      next(error);
    }
