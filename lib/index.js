...
const jwtCommon = require('@lykmapipo/jwt-common');
...
const irinaUtils = require('irina/lib/utils');
...
const PARTY_MODEL_NAME = env.getString('PARTY_MODEL_NAME', 'Party');
const PARTY_COLLECTION_NAME = env.getString('PARTY_COLLECTION_NAME', 'parties');
const DEFAULT_LOCALE = env.getString('DEFAULT_LOCALE', 'en');
const LOCALES = env.getStrings('LOCALES', DEFAULT_LOCALE);
const DEFAULT_PARTY_TYPE = env.getString('DEFAULT_PARTY_TYPE', 'Focal');
const PARTY_TYPES = env.getStringSet('PARTY_TYPES', ['Focal', 'Agency']);
const DEFAULT_PASSWORD = _.trim(env.getString('DEFAULT_PASSWORD', '123456789'));
 ...
 * @private
 */
const PartySchema = new mongooseCommon.Schema(
  {
    /**
     * @name party
      ...
      exportable: {
        format: (v) => {
          return (
            v &&
            v.strings &&
            common.compact([v.strings.name.en, v.strings.abbreviation.en]).join(' - ')
          );
        },
        order: 2,
        default: 'NA',
     ...
     * }
     */
    centre: mongooseGeojsonSchemas.Point,
...
PartySchema.methods.preValidate = function preValidate(done) {
  ...
  this.abbreviation = _.trim(this.abbreviation) || common.abbreviate(this.name);
...
PartySchema.methods.generateToken = function generateToken(done) {
  ...
  const expiresIn = env.getString('JWT_API_TOKEN_EXPIRES_IN', '1000y');
  const payload = { id: common.idOf(party) };
  const { token } = party;

  return jwtCommon.refresh(token, payload, { expiresIn }, (error, jwtToken) => {
    if (error || !jwtToken) {
      const failed = common.normalizeError(
...
PartySchema.methods.asContact = function asContact() {
  const contact = common.mergeObjects({
    name: this.name,
    mobile: this.mobile,
    email: this.email,
...
PartySchema.statics.notify = function notify(notification, done) {
  ...
  // query receivers
  return async.parallel(works, function onGetContacts(error, results) {
    // back off on error
    if (error) {
      return cb(error);
    }
    ...
    _.forEach(emails, function queueNotification(_to) {
      ...
      const SMTP_FROM = env.getString('SMTP_FROM');
      const mail = { sender: SMTP_FROM, to: _to, subject, body };

      // queue emails
      const email = new postman.Email(mail);
      email.queue();
    });
...
PartySchema.statics.findByJwt = function findByJwt(jwt, done) {
  ...
  // execute
  return async.waterfall([findPartyById, verifyParty], done);
};
...
PartySchema.statics.findChildren = function findChildren(criteria, done) {
  ...
  const collectResults = (...updates) => {
    ...
    collected = _.uniqWith(collected, mongooseCommon.areSameInstance);
    return collected;
  };
  ...
  const findKids = (conditions, next) => {
    ...
    return query.exec((error, children) => {
      ...
      if (!_.isEmpty(children)) {
        ...
        const parentIds = _.uniq(mongooseCommon.toObjectIds(...children));
  ...
  const findAncestors = (conditions, next) => {
    ...
    return query.exec((error, ancestors) => {
      ...
      if (!_.isEmpty(ancestors)) {
        ...
        const parentIds = _.uniq(mongooseCommon.toObjectIds(...ancestorIds));
        if (_.isEmpty(parentIds)) {
          return next(null, results);
        }
...
PartySchema.plugin(mongooseKue.plugin);

/* export party model */
const Party = mongooseCommon.model(PARTY_MODEL_NAME, PartySchema);
...
const ensurePassword$1 = (party = {}) => {
  const defaultPassword = env.getString('DEFAULT_PASSWORD', '123456789');
  const password = common.firstValue(defaultPassword, party.password);
  const partyWithPassword = common.mergeObjects(party, { password });
...
const encryptPassword$1 = (party = {}, done) => {
  const localParty = common.mergeObjects(party);
  if (!_.isEmpty(localParty.password)) {
...
const encryptPassword = (request, response, next) => {
  if (common.areNotEmpty(request.body, request.password)) {
...
router.post('/signin', (request, response, next) => {
  ...
  // continue with signin
    ...
    const parsedPhoneNumber = phone.parsePhoneNumber(username);
    let phone$1;

    // check if phone number was parsed
    if (parsedPhoneNumber) {
      phone$1 = parsedPhoneNumber.e164NoPlus;
    }

    const credentials = {
      $or: [{ email }, { mobile: phone$1 || username }],
      deletedAt: {
        $eq: null,
      },
    ...
    async.waterfall(
      [
        ...
        function encodePartyToJWT(party, then) {
          jwtCommon.encode({ id: common.idOf(party) }, function afterEncode(error, jwtToken) {
            if (error) {
              then(error);
            } else {
              then(null, {
      ...
      function done(error, result) {
        ...
        if (error) {
          ...
          common.normalizeError({ status: 403 });
          next(error);
        }
 ...
 */
const jwtAuth = [
  jwtCommon.jwtAuth({ user: (jwt, done) => Party.findByJwt(jwt, done) }),
...
const start = (done) => {
  ...
  mongooseCommon.connect((error) => {
    ...
    expressCommon.mount(permission.permissionRouter);
    expressCommon.mount(predefine.predefineRouter);
...
Object.defineProperty(exports, 'permissionRouter', {
  enumerable: true,
  get: function () {
    return permission.permissionRouter;
  }
});
Object.defineProperty(exports, 'Predefine', {
  enumerable: true,
  get: function () {
    return predefine.Predefine;
  }
});
Object.defineProperty(exports, 'predefineRouter', {
  enumerable: true,
  get: function () {
    return predefine.predefineRouter;
  }
});
exports.Party = Party;
